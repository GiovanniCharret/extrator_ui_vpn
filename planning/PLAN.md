...
| F5 | Laço completo, retomada, CSV | `[x]` DEV / `[ ]` VPN | 15/15 (suíte 41/41) | 17/06/2026 | `src/main.py` (planejar/consolidar/montar+escrever relatório/executar/main) TDD; nome do PDF alinhado ao contrato no `exportar_pdf`; **default re-exporta** (`--retomar` opt-in); `run.ps1` mínimo. `--dry-run` lista 21. Falta a viagem (Roteiro 5) |
| F6 | run.ps1 + documentação final | `[ ]` | — | — | |

### Próximos passos

1. ~~**F2 — preencher o map (DEV, sem viagem)**~~ ✅ **feito**: `config/programas_map.json` preenchido à mão (21 ECO), `pytest tests/test_f2_mapeamento.py` 8/8 (acceptance não pula mais). Resta a validação [VPN] da seleção real no dropdown do LNC (roteiro 4).
2. **F5 (laço/main) depois da F2**: `main.py` — contrato→programa via map, retomada (PDF existe ⇒ pula), consolida todos os PDFs com `parse_pdf` → `output/consolidado.csv`, relatório. F4 já pronta. (Parte testável em DEV pode ser escrita em paralelo à viagem F2.)
