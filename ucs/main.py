...
def main(argv=None):
    ...
    total = consolida.consolidar_csv(mapa, config.RAW_DIR, config.CSV_CONSOLIDADO_UCS, log)  # Fase 4
    if args.sqlite:                                 # opcional: banco + benchmark
        consolida.consolidar_sqlite(mapa, config.RAW_DIR, config.UCS_DB, log)
