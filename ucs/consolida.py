...
# Ordem das colunas da base enxuta de saída (o contrato é prefixado na orquestração).
CABECALHO_SAIDA = ["contrato", "odi", "uc", "cod_projeto", "nome_projeto", "cod_programa"]


def extrair_linhas(caminho):
    """Lê um CSV bruto do SSRS e devolve só as colunas que interessam à base.

    Por que existe: o relatório tem 30 colunas; a base de validação só precisa de
    ODI, UC e alguns campos de rastreio. Isolar a seleção (por NOME de coluna, não por
    posição) deixa a função pura/testável e resistente a reordenação de colunas.
    ...
    Lógica, do input ao output, em fases:
      ...
      Fase 3 — para cada linha, extrai (odi, uc, cod_projeto, nome_projeto, cod_programa).
      Saída: lista de tuplas (odi, uc, cod_projeto, nome_projeto, cod_programa).
    """
    with open(caminho, encoding=config.CSV_IN_ENCODING, newline="") as fh:  # Fase 1: abre (utf-8-sig tira BOM)
        ...
        for linha in leitor:                           # uma linha de dados por vez
            ...
            linhas.append((                            # extrai só as colunas enxutas
                odi,                                   # ODI
                uc,                                    # número da UC
                pega(linha, config.COL_COD_PROJETO),   # código do projeto (rastreio)
                pega(linha, config.COL_NOME_PROJETO),  # nome do projeto (rastreio)
                pega(linha, config.COL_COD_PROGRAMA),  # código do programa (conferência)
            ))
        return linhas                                  # saída: linhas enxutas (sem o contrato)


def consolidar_csv(mapa, raw_dir, caminho_saida, log):
    """Junta todos os brutos presentes num único consolidado_ucs.csv (streaming).

    Por que existe: entrega a base plana (contrato;odi;uc;...) reabrindo cada bruto
    ...
    Lógica, do input ao output, em fases:
      Entrada: mapa (contratos), raw_dir, caminho_saida, log.
      Fase 1 — abre a saída (utf-8-sig, ';') e escreve o cabeçalho.
    ...
    with open(caminho_saida, "w", encoding=config.CSV_ENCODING, newline="") as fh:  # Fase 1
        ...
        escritor.writerow(CABECALHO_SAIDA)             # cabeçalho da base enxuta
        for contrato in mapa:                          # Fase 2: na ordem do mapa
            ...
            if not caminho.exists():                   # sem bruto => contrato não baixado ainda
                ...
                continue
            linhas = extrair_linhas(caminho)           # CSV -> linhas enxutas
...
def consolidar_sqlite(mapa, raw_dir, db_path, log):
    ...
    existe?); um SQLite com índice resolve isso sem carregar 1M linhas. É derivado dos
    brutos (regenerável), então é seguro recriá-lo do zero a cada rodada.

    Lógica, do input ao output, em fases:
      Entrada: mapa, raw_dir, db_path, log.
      Fase 1 — (re)cria a tabela 'ucs' do zero.
    ...
    try:
        ...
        con.execute(                                   # tabela com as colunas da base enxuta
            "CREATE TABLE ucs (contrato TEXT, odi TEXT, uc TEXT, "
            "cod_projeto TEXT, nome_projeto TEXT, cod_programa TEXT)")
